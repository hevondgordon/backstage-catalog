...
class Client_PG extends Client {
  ...
  _parseVersion(versionString) {
    const openSourcePostgres = /^PostgreSQL (.*?)( |$)/.exec(versionString);
    const enterprisePostgres = /^EnterpriseDB (.*?)( |$)/.exec(versionString);

    if (enterprisePostgres !== null) return enterprisePostgres[1];
    if (openSourcePostgres !== null) return openSourcePostgres[1];
  }
